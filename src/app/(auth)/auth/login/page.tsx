...
function LoginForm() {
  ...
  async function handleEmailLogin(e: React.FormEvent) {
    ...
    if (res?.ok) {
      ...
      toast.error("發送失敗，請確認 Email 是否正確");
    }
  }
    ...
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-[500] text-t1 mt-2">登入帳號</h1>
          <p className="text-sm text-t2 mt-1">還沒有帳號？
            <Link href="/auth/register" className="text-ind hover:underline ml-1">免費加入</Link>
          </p>
        </div>
        ...
        <p className="text-xs text-t3 text-center mt-6">登入即代表同意服務條款與隱私政策</p>
      </div>
    </div>
  );
}
